...
private _thirstChange = _currentWork * CHANGE_CONSTANT / GVAR(timeWithoutWater);
private _hungerChange = _currentWork * CHANGE_CONSTANT / GVAR(timeWithoutFood);

// Run status modifiers
{_thirstChange = _thirstChange + (_player call _x) * CHANGE_CONSTANT / GVAR(timeWithoutWater)} forEach GVAR(thirstModifiers);
{_hungerChange = _hungerChange + (_player call _x) * CHANGE_CONSTANT / GVAR(timeWithoutFood)} forEach GVAR(hungerModifiers);
